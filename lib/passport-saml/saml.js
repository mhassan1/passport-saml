...
function callBackWithNameID(nameid, callback) {
    const format = xmlCrypto.xpath(nameid, "@Format");
...
class SAML {
    ...
    validateSignatureForCert(signature, cert, fullXml, currentNode) {
        ...
        const totalReferencedNodes = xmlCrypto.xpath(currentNode.ownerDocument, "//*[@" + idAttribute + "='" + refId + "']");
    ...
    validatePostResponse(container, callback) {
        ...
        (async () => {
            ...
            const inResponseToNodes = xmlCrypto.xpath(doc, "/*[local-name()='Response']/@InResponseTo");
            ...
            const encryptedAssertions = xmlCrypto.xpath(doc, "/*[local-name()='Response']/*[local-name()='EncryptedAssertion']");
    ...
    getNameID(self, doc, callback) {
        ...
        const encryptedIds = xmlCrypto.xpath(doc, "/*[local-name()='LogoutRequest']/*[local-name()='EncryptedID']");
    ...
    keyToPEM(key) {
        ...
        const wrappedKey = [
            '-----BEGIN PRIVATE KEY-----',
            ...((_a = key.match(/.{1,64}/g)) !== null && _a !== void 0 ? _a : []),
            '-----END PRIVATE KEY-----',
            ''
        ].join('\n');
        return wrappedKey;
    }
    normalizeNewlines(xml) {
